class NotificationMailers < ActionMailer::Base
  ...
  def notifications_digest user
    ...
    mail(from: "#{@user.household.nickname} Expenses (Splitzies.com) <team@benbergstein.mailgun.org>", to: "#{@user.first_name} #{@user.last_name} <#{@user.email}>", subject: "#{user.notify_every)
  end
end
